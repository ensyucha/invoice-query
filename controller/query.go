...
func processQueryArray(queryArray *model.QueryArray, user *model.User) context.Map {
	...
	for {
		...
		case queryResult := <- queryResultChan:
			...
			if judgeQuerySuccess(queryResult.RespCode) { // 查询成功

				err := dbop.DeleteUncheckData(&model.Query {
					Fpdm: queryResult.Data.Fpdm,
					Fphm: queryResult.Data.Fphm,
					Kprq: queryResult.Data.Kprq,
				}, user.Username)
				...
				queryResult.RespMsg = `<span class="my-success">` + queryResult.RespMsg + `</span>`
				...
				queryResult.RespMsg = `<span class="my-failed">` + queryResult.RespMsg + `</span>`

				err := dbop.UpdateFailedCheckData(&model.Query {
					Fpdm: queryResult.Data.Fpdm,
					Fphm: queryResult.Data.Fphm,
					Kprq: queryResult.Data.Kprq,
					Je:   "-1",
				}, queryResult.RespMsg, user.Username)
				...
				if err != nil {
					...
					if len(body) != 0 {
						...
						if err != nil {
							...
							queryResult.RespMsg = `Client请求超时，请稍后再试`
							dbop.WriteLog("system", "3002："+err.Error(), "system")
						}
					} else {
						queryResult.RespCode = "3003"
						queryResult.RespMsg = `联系管理员检查系统额度`
